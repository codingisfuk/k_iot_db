...
CREATE TABLE `student_coursr_connects` (
	# 학생/과목 관계 테이블
    student_id int,
    course_id int,
    primary key (student_id, course_id),
    foreign key (student_id) references students(stident_id),
...
SELECT * FROM COURSES;
SELECT * FROM STUDENTS_COURSE_CONNECTS;
...
delete from STUDENTS_COURSE_CONNECTS
where
	student_id = 3;
